        ...
        case 'list_directory_response':
            ...
            if (message.request_id) {
                ...
                if (requestingElement) {
                    // Verifica se la richiesta è ancora l'ultima attiva per questo elemento
                    // Non serve un nuovo lastListDirectoryRequestId globale qui, perché l'ID è già associato
                    // all'elemento specifico del treeview (directoryElement.dataset.requestId)
                    // e viene rimosso dopo l'elaborazione.
                    // Se message.request_id non è nullo, e requestingElement è trovato,
                    // e requestingElement.dataset.requestId corrisponde a message.request_id,
                    // allora la risposta è valida per questo elemento.
                    // Altrimenti, se requestingElement.dataset.requestId è stato rimosso o modificato,
                    // la risposta è obsoleta.
                    ...
                    requestingElement.removeAttribute('data-request-id'); // IMPORTANTE: Rimuove l'ID
                } else {
                     console.warn("Treeview - Risposta list_directory_response senza elemento richiedente corrispondente per ID:", message.request_id);
                     notifyParentMessage(`Avviso: Risposta directory ricevuta senza ID richiesta corrispondente.`, 'warning');
                 }
            } else {
                console.warn("Treeview - Risposta list_directory_response ricevuta senza RequestID.");
